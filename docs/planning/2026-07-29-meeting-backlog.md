...
| Id | Item | Situacao |
|---|---|---|
| **N16** | **Validacao de cliente menor de 18 anos** | **Diagnosticado 2026-07-31: NAO E BUG.** A flag `service_types.requires_age_verification` (M7) e a validacao de idade no backend estao corretas e testadas (`assert-age-verification.ts`, calculo exato mes/dia, bloqueio com 422). O problema: **nao existe NENHUMA UI pra ligar a flag** — nem no dialog de criar tipo de servico, nem no de editar (so tem campo Nome/Descricao). Unica forma de ligar hoje e chamada HTTP direta. Como toda org fica com a flag `false` por padrao e sem UI pra mudar, o teste do Ruan nao podia dar outro resultado. | 03:04–04:05 |
| **N17** | **Renomear arquivo depois do upload** | **Diagnosticado 2026-07-31: NUNCA foi implementado** (nao e regressao). M7/F7 entregou so a metade "nome amigavel no download" (`file_name` separado de `storage_path`, usado no `Content-Disposition`). Nao existe endpoint (`PATCH`), use-case, mutation no hook nem botao/acao na UI de anexos do cliente — so upload e delete. | 04:05–04:41 |

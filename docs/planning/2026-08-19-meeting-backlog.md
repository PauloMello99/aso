...
| ~~**P-2 fatia 3/4 — Design + frontend das 3 telas**~~ | ✅ **Concluída (2026-08-20)**, em worktree dedicado (`p2-fatia3-customer-self-service-frontend`, branch `worktree-p2-fatia3-customer-self-service-frontend`, ainda não commitada/mergeada). Menu no módulo de Clientes (header: cadastro manual / criar+ficha via link; ação por linha: enviar atualização cadastral) + 2 páginas públicas novas (`/customer-registration/:token` form de 2 passos dados+anamnese; `/customer-update/:token` form de 1 passo com atualização PARCIAL via `dirtyFields`, cuidado especial pra nunca apagar o gênero do cliente se o campo não for tocado — GET não devolve gênero atual). Revisada por `reviewer` (`approved_with_notes`, nenhum crítico; 2 findings low identificados eram pré-existentes fora de escopo, confirmado via `git diff`). Validado: `check-types`/`lint`/`test` (33 suites/255 testes)/`build` (rotas novas geradas) + verificação visual real no browser (estado "link inválido" renderiza corretamente nas 2 páginas públicas, mobile 375px ok, auth guard intacto). **Falta**: commit da fatia (aguardando decisão do usuário). | complexa (tela nova, consumindo endpoints públicos já entregues) | Concluído antes do Ink Day |

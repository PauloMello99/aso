...
| ~~**P-2 fatia 3/4 — Design + frontend das 3 telas**~~ | ✅ **Concluída (2026-08-20)**, commit `4c7f308` na branch `worktree-p2-fatia3-customer-self-service-frontend`, mergeada em `development`. Menu no módulo de Clientes (header: cadastro manual / criar+ficha via link; ação por linha: enviar atualização cadastral) + 2 páginas públicas novas (`/customer-registration/:token` form de 2 passos dados+anamnese; `/customer-update/:token` form de 1 passo com atualização PARCIAL via `dirtyFields`, cuidado especial pra nunca apagar o gênero do cliente se o campo não for tocado — GET não devolve gênero atual). Revisada por `reviewer` (`approved_with_notes`, nenhum crítico). Validado: `check-types`/`lint`/`test` (33 suites/255 testes)/`build` (rotas novas geradas) + verificação visual real no browser (estado "link inválido" renderiza corretamente nas 2 páginas públicas, mobile 375px ok, auth guard intacto).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | complexa (tela nova, consumindo endpoints públicos já entregues)              | Sem prazo fixo — considerar antes do Ink Day se a agenda permitir (cenário 3 também apoia a migração V1, ver P-3) |
| ~~**P-2 — pendências finais (achados `low` do reviewer)**~~ | ✅ **Concluída (2026-08-20)**, commit `3cc1e15`. `fetchAddressByCep` deduplicado em `shared/lib/viacep.ts` (3 consumidores); hint de gênero preservado no Select da página pública de atualização; 4º estado `error` em `public-lookup-state.ts` (falha de transporte ≠ token inválido) nas 2 páginas públicas, com botão de tentar novamente. Validado: `check-types`/`lint`/`test` (33 suites/263 testes)/`build`. **P-2 está integralmente concluída e mergeada em `main`** (via `staging`/`development`, PRs #59/#60). | low (follow-up de review) | Concluído |
...
## Status geral do ciclo (2026-08-20)

Todas as milestones de **código** deste backlog estão **concluídas e em `main`**
(P-BUG-1, P-1, P-1-FIX, P-2 fatias 1-4). Não há trabalho de desenvolvimento pendente aberto
nesta reunião. O que resta até o Ink Day (12-13/09) é:

- **P-3** — ETL de migração Ink House V1 → ASO (execução técnica do Paulo, fora de milestone
  de produto) + teste de dupla-entrada do Ruan (em andamento, responsabilidade dele).
- **P-4** — ajuste operacional de preço/cupom no painel `/admin` de billing (sem código).
- **P-5** — consulta jurídica do JP sobre marca/CNPJ (sem código).

Se surgir uma nova tarefa de desenvolvimento, ela não pertence a este backlog — abrir um novo
arquivo de planejamento ou registrar diretamente em `.memory/roadmap.md`.

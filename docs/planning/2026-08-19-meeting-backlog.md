...
|---|---|---|---|
| **P-BUG-1 — Fix visibilidade do Caixa p/ super_admin com membership real** | `org-layout.tsx` não deve sobrescrever `role` para "owner" quando o super_admin tem membership real de `employee` na org — nav/rotas devem seguir a permissão real (decisão fechada: rota sem acesso não aparece nem é acessível, não é só ajuste de banner). | intermediária/complexa (auth/permissões) | Curto prazo — bug e decisão já confirmados, pronto para `locator`/`planner` |
| **P-1 — Comissão/repasse por profissional** | Config de percentual + modo pré/pós-taxa por `(org_id, member_id)` com **histórico** (tabela própria, não coluna sobrescrita) — serviço criado guarda snapshot do percentual e do modo vigentes à época. Telas de visualização (profissional: movimentado × líquido; owner: por profissional). Sem fluxo de pagamento real. | complexa (dinheiro/caixa + nova tabela com histórico) | Antes do Ink Day (12-13/09) — pedido explícito do Ruan |
| **P-2 — Auto-cadastro/atualização de cliente no módulo de Clientes** | 3 cenários no módulo de Clientes (não no sheet de serviço), 3 recursos públicos distintos: (1) `public/customer-registrations/:token` — e-mail+tipo de serviço informados pelo profissional (com checagem de e-mail já cadastrado na org), cria `customer`+`anamnesis_response` atomicamente; (2) `public/anamnesis-responses/:token` — inalterado (M10b); (3) `public/customer-updates/:token` — novo, atualiza só dados cadastrais de um `customer` existente. | complexa (dois endpoints novos de escrita pública, atomicidade entre módulos, tela nova) | Sem prazo fixo citado — considerar antes do Ink Day se a agenda permitir (cenário 3 também apoia a migração V1, ver P-3) |

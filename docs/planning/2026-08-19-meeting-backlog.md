...
selecionado — mas isso não significa que o *gatilho* do auto-cadastro deva morar ali. A
...
| ~~**P-1 — Comissão/repasse por profissional**~~ | ✅ **Concluído (2026-08-19)**, commits `dbe3613` (backend) e `af117d7` (frontend). Config por `(org_id, member_id)` com histórico imutável (`org_member_commissions`, supersede append-only), snapshot desnormalizado gravado em `services` no momento do pagamento, dois triggers de banco protegendo a imutabilidade pós-pagamento. Telas: owner configura em Configurações › Caixa; overview mostra "Minha comissão" (employee) e "Comissão a repassar" + repasse por profissional (owner). Revisado por `database-guardian` (4 rodadas) e `reviewer` (3 rodadas). | complexa (dinheiro/caixa + nova tabela com histórico) | Concluído antes do Ink Day |
| 🚧 **P-2 — Auto-cadastro/atualização de cliente no módulo de Clientes** | **Em andamento (2026-08-19)** — fatia 2/4 concluída (backend completo, ainda não commitada nesta sessão): módulo `customer-self-service` (domain/infra/use-cases/controllers), `anamnesis.module.ts` corrigido (`exports` + `linkCustomer(responseId, customerId, orgId)`), 2 templates de e-mail novos, 6 use-cases (2 autenticados para gerar convite + 4 públicos GET/submit dos cenários 1 e 3), `IPublicCustomerWriter`/`DrizzlePublicCustomerWriter` (DRIZZLE_ADMIN escopado, estende padrão ADR-0021 para `customers`), 2 controllers (autenticado `orgs/:orgId` + público `public/...`), módulo registrado em `app.module.ts`. Revisado por `database-guardian` (1 rodada, corrigida) e `reviewer` (1 rodada, corrigida — retry por marcador `customerId` em vez de e-mail, e-mail exposto no DTO do cenário 3, `serviceTypeId` no reuso de convite, `orgId` em `linkCustomer`, `changedFields` do audit). Validado: `check-types`/`lint`/`test` (596/596)/`build` + boot real do Nest com as 6 rotas mapeadas e DI resolvida. Ver seção "M-P2b" em `.memory/domain-rules.md`. **Faltam**: `design` das 3 telas novas, frontend (hooks, páginas públicas `/customer-registration/:token` e `/customer-update/:token`, integração no módulo de Clientes), `reviewer` final do frontend, commit da fatia 2/4. **Nota de risco**: as páginas públicas ainda não existem — não devem mergear só com validação estática; falta verificação visual real em navegador. | complexa (dois endpoints novos de escrita pública, atomicidade entre módulos, tela nova) | Sem prazo fixo citado — considerar antes do Ink Day se a agenda permitir (cenário 3 também apoia a migração V1, ver P-3) |

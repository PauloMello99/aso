...
| 🚧 **P-2 — Auto-cadastro/atualização de cliente no módulo de Clientes** | **Em andamento (2026-08-19)** — fatia 1/4 commitada (`3725767`): schema Drizzle + migrations 0052/0053 (tabelas `customer_self_registrations`/`customer_update_invitations`, FK composta `(customer_id, org_id)` para impedir referência cross-org, índices únicos parciais, policies RLS, `customers_id_org_id_uq`), validadas via `database-guardian` (2 rodadas) e `db:rollback`/`db:migrate`/`db:status`. **Faltam**: camada de domínio/infra do módulo `customer-self-service` (entidades, repos, exceções), correção do `anamnesis.module.ts` (falta `exports`), use-cases dos 3 cenários (autenticado + público), controllers/DTOs, `database-guardian` da camada de aplicação, `design` das 3 telas novas, frontend (hooks, 2 páginas públicas, integração no módulo de Clientes), `reviewer` final. Ver plano de 18 passos do `planner` e pendências registradas no corpo do commit `3725767`. **Nota de risco**: as 2 páginas públicas novas são superfície não-autenticada — não devem mergear só com validação estática; falta ambiente para verificação visual real (porta 3000 ocupada por processo Docker não relacionado nesta sessão, sem credenciais de login seedadas). | complexa (dois endpoints novos de escrita pública, atomicidade entre módulos, tela nova) | Sem prazo fixo citado — considerar antes do Ink Day se a agenda permitir (cenário 3 também apoia a migração V1, ver P-3) |

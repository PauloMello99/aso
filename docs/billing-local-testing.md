...
| Desconto (admin) | `POST /admin/orgs/:orgId/subscription/discount` numa org já vinculada ao Stripe | Cupom real criado e aplicado (visível no dashboard Stripe, modo teste); o espelho local `SELECT stripe_coupon_id, discount_percent FROM subscriptions WHERE org_id=…` reflete o cupom (percentual inteiro → `discount_percent`; `amount_off` ou fracionário → id do cupom com `discount_percent` `NULL`) |
| Desconto — espelho (Stripe → local) | Numa org **vinculada ao Stripe e não-cortesia** (`type <> 'custom'` — `shouldApplyStripeSync` nunca sincroniza cortesia), aplicar um cupom direto pelo Dashboard do Stripe (modo teste), no nível da subscription → rodar `POST /internal/cron/tick` (`x-cron-secret`; job `billing-reconciliation`) **ou** deixar o `customer.subscription.updated` chegar pelo `stripe listen` | `subscriptions.stripe_coupon_id`/`discount_percent` passam a refletir o Stripe (mapeamento como na linha acima). No primeiro reconcile pós-correção há um `telemetry` `BILLING_SUBSCRIPTION_DISCOUNT_DRIFT_OVERWRITTEN` por org sobrescrita (esperado). Remover o cupom no Dashboard e repetir → os dois campos voltam a `NULL` sozinhos. Cupom no **nível do item** (não da subscription) → não aparece em `subscription.discounts`, mapeia para os dois `NULL`. **Round-trip ao vivo NÃO exercitado nesta entrega** (chave do `stripe` CLI expirada) — cobertura por unit test (`stripe-payment-gateway.spec.ts`, `reconcile-subscriptions.use-case.spec.ts`). Ver `.memory/adr/0016-billing-stripe-assinatura.md` (Addendum 2026-08-31 — espelho fiel do desconto do Stripe) |

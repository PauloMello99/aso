...
| ADR-0033 | Aviso de novidades por e-mail aos donos guiado por SemVer (Bloco 5.2 fatia C): relevância DERIVADA (`shouldNotifyOwners` major/minor + corte `NOTIFY_FROM_VERSION=4`, sem flag `notifyOwners`), log append-only `changelog_notifications` (0083, UNIQUE user+entry, falha terminal, `error` só redigido), opt-out `users.product_updates_opted_out_at` (0082, expõe só booleano), alvos = donos reais habilitados de org ativa, use-case com kill-switch + canal + `claimRun` 10 min + allowlist antes de gravar + teto 50/varredura 500; desligar = kill-switch, não rollback |
| ADR-0032 | Onboarding modular (Bloco 5.2 fatia A): progresso por usuário em `users.onboarding_seen jsonb` (migration 0081, merge no servidor, CHECK ≤4096 B), `visto = seen[id] >= version OU completedAt >= introducedAt` (sem backfill), registro explícito `ONBOARDING_MODULE_META` por href de nav (falha se faltar), fechar por qualquer via grava TODOS os oferecidos, replay `?tour=1` não grava; migrate antes do backend |
...
| ADR-0030 | Changelog versionado (Bloco 5.2 fatia B): itens em arquivo do repo (`changelog-entries.ts`, seed histórico sempre `notifyOwners:false`), estado visto = high-water mark `users.changelog_seen_version` (migration 0080, escrita só via `updateChangelogSeenVersion` com `GREATEST`), `GET /changelog` + `POST /changelog/seen` autenticados e não org-scoped, filtro de audience/módulo no frontend, banner dismissível no `OrgLayout`; pré-requisito de deploy: migrate antes do backend |
| ADR-0029 | Alerta de estoque baixo (Bloco 5.1): in-app + e-mail imediato disparado pelas ESCRITAS de estoque (sem cron), detecção por transição via marcador `materials.low_stock_alerted_at` recalculado no mesmo UPDATE + `FOR UPDATE` no choke-point `updateStockQuantity`; envio pós-COMMIT destacado (não awaited) agrupado por operação, 1 notificação por dono; migrations 0078/0079 |
| ADR-0028 | Campanhas (Bloco 4): trava de allowlist de e-mail fora de produção (`APP_ENVIRONMENT !== "production"` ⇒ enforcing, lista vazia bloqueia tudo, gate no use-case antes de gravar qualquer linha), webhook de bounce do Resend em endpoint/segredo próprios (`/webhooks/campaign-delivery`, `handled:false`⇒200, idempotência pela UNIQUE de `campaign_sends`, correlação por tag `campaign_send_id`), migrations 0076 (audit_action) e 0077 (policy SELECT owner-only em `campaign_sends`, supersede decisão (f) da 0063) e relatório de entrega owner-only com `WHERE org_id` explícito | 2026-09-19 | Aceito |

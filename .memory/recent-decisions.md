...
| ADR-0034 | Pagamento a membro: `transactions` permanece **agnóstica** (nenhuma coluna de beneficiário), a marcação "esta transação é o pagamento do membro X" vive numa **entidade própria isolada e append-only** que referencia a transação; correção = linha de reversão + estorno no caixa na mesma ação (ADR-0010), nunca `UPDATE`/`DELETE`; tela do membro é só leitura, só o clique do owner em "Pagar" lança. **Reverte** `docs/planning/2026-08-19-meeting-backlog.md` item 3 ("não criar fluxo de pagamento real ao profissional") | 2026-09-16 | Aceito |
| ADR-0033 | Aviso de novidades por e-mail aos donos guiado por SemVer (Bloco 5.2 fatia C): relevância DERIVADA (`shouldNotifyOwners` major/minor + corte `NOTIFY_FROM_VERSION=4`, sem flag `notifyOwners`), log append-only `changelog_notifications` (0082, UNIQUE user+entry, falha terminal, `error` só redigido), opt-out `users.product_updates_opted_out_at` (0081, expõe só booleano), alvos = donos reais habilitados de org ativa, use-case com kill-switch + canal + `claimRun` 10 min + allowlist antes de gravar + teto 50/varredura 500; desligar = kill-switch, não rollback |
| ADR-0032 | Onboarding modular (Bloco 5.2 fatia A): progresso por usuário em `users.onboarding_seen jsonb` (migration 0080, merge no servidor, CHECK ≤4096 B), `visto = seen[id] >= version OU completedAt >= introducedAt` (sem backfill), registro explícito `ONBOARDING_MODULE_META` por href de nav (falha se faltar), fechar por qualquer via grava TODOS os oferecidos, replay `?tour=1` não grava; migrate antes do backend |
| ADR-0031 | Versionamento SemVer UNIFICADO do produto (aceito, base 1.0.0): versão única na raiz + apps em lockstep (packages 0.0.0 fora), único caminho `pnpm version:bump` (`bin/scripts/release/version.mjs`), `version:check` no CI + spec de guard + skill `product-versioning`; minor/major ⇒ item de changelog com o mesmo `semver`, patch nunca tem item; `notifyOwners` substituído por derivação (major/minor) + corte `NOTIFY_FROM_VERSION=4` |
| ADR-0030 | Changelog versionado (Bloco 5.2 fatia B): itens em arquivo do repo (`changelog-entries.ts`, seed histórico sempre `notifyOwners:false`), estado visto = high-water mark `users.changelog_seen_version` (migration 0079, escrita só via `updateChangelogSeenVersion` com `GREATEST`), `GET /changelog` + `POST /changelog/seen` autenticados e não org-scoped, filtro de audience/módulo no frontend, banner dismissível no `OrgLayout`; pré-requisito de deploy: migrate antes do backend |
| ADR-0029 | Alerta de estoque baixo (Bloco 5.1): in-app + e-mail imediato disparado pelas ESCRITAS de estoque (sem cron), detecção por transição via marcador `materials.low_stock_alerted_at` recalculado no mesmo UPDATE + `FOR UPDATE` no choke-point `updateStockQuantity`; envio pós-COMMIT destacado (não awaited) agrupado por operação, 1 notificação por dono; migrations 0077/0078 |
| ADR-0028 | Campanhas (Bloco 4): trava de allowlist de e-mail fora de produção (`APP_ENVIRONMENT !== "production"` ⇒ enforcing, lista vazia bloqueia tudo, gate no use-case antes de gravar qualquer linha), webhook de bounce do Resend em endpoint/segredo próprios (`/webhooks/campaign-delivery`, `handled:false`⇒200, idempotência pela UNIQUE de `campaign_sends`, correlação por tag `campaign_send_id`), migrations 0075 (audit_action) e 0076 (policy SELECT owner-only em `campaign_sends`, supersede decisão (f) da 0063) e relatório de entrega owner-only com `WHERE org_id` explícito | 2026-09-19 | Aceito |
| ADR-0027 | Taxa de cartão de crédito parcelado: `installments` como ATRIBUTO (não enum novo, não catálogo de condições) em `org_payment_fees`/`org_member_payment_fees` (config, NOT NULL DEFAULT 1) e `transactions`/`services` (negócio, nullable, 3 significados de NULL); chave composta `(org,method,installments)`/`(org,user,method,installments)`; SEM fallback entre faixas (falta config = taxa zero, mitigado só por invariante de UI); correção de faixa honra taxa do MEMBRO; `MAX_INSTALLMENTS=12` na UI (CHECK do banco até 24) | 2026-09-18 | Aceito |

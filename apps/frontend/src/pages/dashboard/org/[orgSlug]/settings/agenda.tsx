...
const EXTERNAL_CALENDARS = [
  ...
  { name: "Apple Calendar", color: "text-foreground/70" },
]
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-3">
        {EXTERNAL_CALENDARS.map((c) => (
          <button
            ...
            type="button"
            disabled
            className="flex cursor-not-allowed items-center justify-between rounded-lg border border-foreground/[0.06] bg-foreground/[0.02] px-3 py-2.5 text-sm text-foreground/60 opacity-70"
          >
            <span className={c.color}>{c.name}</span>
            <span className="text-xs text-foreground/30">Conectar</span>
          </button>
        ))}
      </div>
    </section>

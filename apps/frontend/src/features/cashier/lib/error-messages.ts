...
const COMMISSION_GENERIC_MESSAGE = "Não foi possível salvar a comissão.";

const COMMISSION_CODE_MESSAGES: Record<string, string> = {
  CASHIER_FORBIDDEN: "Você não tem permissão para configurar comissões.",
  COMMISSION_MEMBER_NOT_FOUND:
    "Este membro não está mais ativo na organização.",
};

export function commissionErrorMessage(err: unknown): string {
  if (err instanceof ApiError) {
    if (err.code) {
      const mapped = COMMISSION_CODE_MESSAGES[err.code];
      if (mapped) return mapped;
      // client.ts já traduz SUBSCRIPTION_REQUIRED para uma mensagem pt-BR
      // específica em ApiError.message — não engolir com o fallback genérico.
      if (err.code === "SUBSCRIPTION_REQUIRED") return err.message;
    }
    return COMMISSION_GENERIC_MESSAGE;
  }
  if (err instanceof Error) return err.message;
  return COMMISSION_GENERIC_MESSAGE;
}

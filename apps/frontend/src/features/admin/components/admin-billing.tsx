  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <Label htmlFor="admin-billing-org-search">Organização</Label>
        <div className="relative">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-foreground/30" />
          <Input
            id="admin-billing-org-search"
            value={selectedOrg ? `${selectedOrg.name} (/${selectedOrg.slug})` : query}
            onChange={(e) => {
              setSelectedOrgId(null)
              setQuery(e.target.value)
            }}
            onFocus={() => setSelectedOrgId(null)}
            placeholder="Buscar organização por nome ou slug…"
            className="pl-9"
          />
        </div>

        {!selectedOrg && query.trim() !== "" && (
          <div className="max-h-64 overflow-y-auto rounded-lg border border-foreground/[0.06]">
            {filteredOrgs.length === 0 ? (
              ...
              filteredOrgs.map((o) => (
                <button
                  ...
                  className="flex w-full items-center gap-2 border-b border-foreground/[0.04] px-4 py-2.5 text-left text-sm last:border-0 hover:bg-foreground/[0.04]"
                >
                  <Building2 className="h-4 w-4 shrink-0 text-foreground/30" />
                  <span className="font-medium text-foreground">{o.name}</span>
                  <span className="text-foreground/40">/{o.slug}</span>
                </button>
              ))
            )}
          </div>
      ...
      {selectedOrg && <OrgSubscriptionPanel org={selectedOrg} />}
    </div>
  )
}

function OrgSubscriptionPanel({ org }: { org: AdminOrg }) {
